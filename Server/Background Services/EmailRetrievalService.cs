            ...
            using (var client = new ImapClient())
            {
                ...
                if (_context.Emails.Count() == 0)
                {
                    if (inbox != null)
                    {
                        ...
                        _context.Emails.Add(new Email { Sender = message.From.ToString(), Subject = message.Subject, Body = message.HtmlBody.ToString(), DateRecieved = message.Date.LocalDateTime, Bcc = message.Bcc.ToString(), Cc = message.Cc.ToString(), RepliedToID = Convert.ToInt32(message.InReplyTo), Deleted = false });
                    }
                }
                else
                {
